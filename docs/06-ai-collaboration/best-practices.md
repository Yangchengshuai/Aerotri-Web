...
> **基于 Vibe Coding 实战经验总结的 35 条实战技巧**
...
**问题**: 同源模型（如全 Claude 生态）容易遇到思维瓶颈，即使划分不同 agent 角色也容易出现信息茧房效应。

**解决方案**: 引入不同架构模型（Claude + GPT/Codex）进行互补。

#### 推荐模型分工策略

**规划阶段**:
- **Opus 4.6**: 制定方案和技术架构
- **Codex GPT-5.3**: 挑刺和批判，识别方案盲点

**实施阶段**:
- **Sonnet 4.6 (编码)**: 负责具体代码实现
- **Sonnet 4.6 (审核)**: 初步审核代码逻辑和规范
- **Codex 5.3 High**: 最终质量审核，发现潜在 bug
...
- 时间成本：多轮审核耗时较长
- Token 成本：多次模型调用费用较高
- 沟通成本：Codex 可能给 Claude review 出很多"伪 bug"（风格差异）

#### Claude Code 调用 Codex 的两种方式

**方式 1: MCP（Model Context Protocol）**
- 优点：标准化协议
- 缺点：性能开销较大

**方式 2: Skill（推荐）** ⭐
- 优点：性能更好，更灵活
- GitHub: [skill-codex](https://github.com/your-org/skill-codex)

**实践建议**:
```
开发流程：Opus 规划 → Codex 挑刺 → Sonnet 编码 → Sonnet 初审 → Codex 终审
适用场景：关键模块、核心算法、复杂功能
普通功能：可简化为 Sonnet 编码 + Sonnet 审核
```
...
**解决方案**:
Claude Code 插件，自动捕获工具调用、生成语义摘要、跨会话注入相关上下文。
...
**效果对比**:
| 项目切换恢复 | 15-30 分钟 → 0 分钟（自动） |
| 查找历史决策 | 15-30 分钟 → 3-5 秒 |
| 理解代码意图 | 1-2 小时 → 10 分钟 |
