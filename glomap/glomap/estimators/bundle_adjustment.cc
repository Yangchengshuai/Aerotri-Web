...
bool BundleAdjuster::Solve(const ViewGraph& view_graph,
  ...
  if (cuda_solver_enabled) {
    ...
    LOG(INFO) << "Bundle adjustment: Using GPU cuDSS acceleration (GPU index: " 
              << gpu_indices[0] << ", images: " << num_images << ")";
  } else if (options_.use_gpu) {
    LOG(INFO) << "Bundle adjustment: GPU requested but not enabled "
              << "(images: " << num_images 
              << ", min_required: " << options_.min_num_images_gpu_solver << ")";
  } else {
    LOG(INFO) << "Bundle adjustment: Using CPU-based sparse solvers";
  }
#else
  if (options_.use_gpu) {
    LOG_FIRST_N(WARNING, 1)
